...
//       try {
//         const response = await fetch("http://127.0.0.1:8000/api/storefeedback/", {
//           method: "POST",
//           headers: {
  ...
  const sendemail = async()=>{
    const response = await fetch("http://127.0.0.1:8000/api/send-email/", {
      ...
      body: new URLSearchParams({
        ...
        message : "the student has been terminated because we found multiple tab switches, so found it is a malparctice"
      }),
    });

    const res = await response.json();
    alert("email sent to respected teacher now ur in trouble")
  }
